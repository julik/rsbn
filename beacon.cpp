...
double Beacon::distanceToBeacon(XPLMDataRef acfX, XPLMDataRef acfY, XPLMDataRef acfZ)
{
    ...
    double acfX = XPLMGetDataf(acfX);
	double acfY = XPLMGetDataf(acfY);
	double acfZ = XPLMGetDataf(acfZ);
	...
	double dist = abs(sqrt( distXYsq + pow(relZ, 2) ));
    return dist;
}
...
double Beacon::bearingToAcf(XPLMDataRef acfLat, XPLMDataRef acfLon)
{
    ...
    double lat2 = XPLMGetDataf(rsbnAcfLat);
    double lon2 = XPLMGetDataf(rsbnAcfLon);
